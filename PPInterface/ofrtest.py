...
class OpenFoldWraper:
    ...
    def inference_monomer(self, pdb_df, n_recycle=2):#, template_mask=True, side_chain_mask=False, return_all_cycles=False):
        ...
        loss = torch.sum(out_batch["plddt"])
        print(loss)
        print(list(out_batch))
...
if __name__ == "__main__":
    ...
    pdb_df = load_protein("../example/helix.pdb")
    ofr = OpenFoldWraper(weights_path = "../data/params_model_2_ptm.npz", device="mps")
    of_output, _, pdb_df_pred,  = ofr.inference_monomer(pdb_df)
    save_pdb(pdb_df, "../test.pdb")
